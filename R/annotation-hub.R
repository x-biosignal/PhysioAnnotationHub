...
loadAnnotationHub <- function(reload = FALSE) {
  ...
  hub <- list(
    ...
    icf = read.csv(file.path(extdata, "clinical_icf.csv"),
                   stringsAsFactors = FALSE),
    icf_core_sets = read.csv(file.path(extdata, "icf_core_sets.csv"),
                             stringsAsFactors = FALSE),
    metric_icf = read.csv(file.path(extdata, "metric_icf_map.csv"),
                          stringsAsFactors = FALSE)
  )
...
print.PhysioAnnotationHub <- function(x, ...) {
  ...
  cat("ICF Core-Set Categories:", nrow(x$icf_core_sets), "\n")
  cat("Metric-ICF Links:", nrow(x$metric_icf), "\n")
  invisible(x)
}
